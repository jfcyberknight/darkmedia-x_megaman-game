...
| Delta physique | **clampé à 33,4 ms** (installé dans `GameScene.create()`, garde `_deltaClamped`) | Sous ~30 fps, le jeu **ralentit** au lieu de laisser les corps sauter à travers les tuiles (tunneling). Pire cas : chute `maxFall` 260 px/s × 33 ms ≈ 8,7 px < tuile 16 px. ⚠️ `game.physics` n'existe pas dans Phaser 3 : `physics` s'injecte par scène, et patcher `world.update` depuis `main.ts` plante le boot. |
...
3. Le clamp delta vit dans `GameScene.create()` (wrap de `world.update` avec
   garde `_deltaClamped`). Si vous créez un second monde physique, appliquer
   le même clamp.
